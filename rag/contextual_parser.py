...
class Parsers:
    ...
    def chipper(self, contents: bytes) -> list[tuple[str, dict]]:
        ...
        # Writing parsed data into a file
        with open("mini_parsed_docs2.json", "a", encoding="utf-8") as file:
            for text, metadata in docs:
                    json.dump({"text": text, "metadata": metadata}, file, ensure_ascii=False)
                    file.write("\n")  # Writes each entry on a new line
        
        # with open("mini_parsed_docs2.json", "a", encoding="utf-8") as file:
        #     json.dump([{"text": text, "metadata": metadata} for text, metadata in docs], file, ensure_ascii=False, indent=4)
    ...
    def process_document_with_context(self, docs) -> list[tuple[str, dict]]:
        ...
        # Write enhanced documents to file
        with open("mini_enhanced_docs.json", "a", encoding="utf-8") as file:
            for text, metadata in enhanced_docs:
                json.dump({"text": text, "metadata": metadata}, file, ensure_ascii=False)
                file.write("\n")  # Writes each entry on a new line
        
        # Write enhanced documents to a JSON file
        # with open("mini_enhanced_docs.json", "a", encoding="utf-8") as file:
        #     json.dump([{"text": text, "metadata": metadata} for text, metadata in enhanced_docs], file, ensure_ascii=False, indent=4)
    ...
    def __call__(self, contents: bytes) -> list[tuple[str, dict]]:
        ...
        # parsed_output = self.mock_unstructured_request(contents)
        chipper_parsed_output = self.load_chunks("mini_parsed_docs2.json")
        print("loaded")
        return chipper_parsed_output
        
        # contextual_chunks = self.load_chunks("alphabet_enhanced.json")
        # print("loaded")
    ...
    def load_chunks(self, filename: str) -> list[tuple[str, dict]]:
        """
        Reads data from a saved JSON file and returns it in the format of a list of tuples
        containing text chunks and metadata.

        Args:
            filename: The path to the JSON file containing parsed document data.

        Returns:
            A list of tuples where each tuple contains a text chunk and its associated metadata.
        """
        # try:
        ...
        #     # Remove any instances of '][' between JSON objects
        #     cleaned_data = re.sub(r'\]\s*\[', ',', data)
            
        #     # Load the cleaned data as a JSON list
        #     json_data = json.loads(cleaned_data)

        #     # Format the data into a list of tuples
        #     parsed_data = [(item["text"], item["metadata"]) for item in json_data]
        #     return parsed_data
